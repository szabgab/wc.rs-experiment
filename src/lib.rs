...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(result, Wc { rows: 0, words: 0, bytes: 0} );
        assert_eq!(result.rows, 0);

        let result = wc("files/hello.txt");
        assert_eq!(result, Wc { rows: 0, words: 2, bytes: 11 });

        let result = wc("files/hello_with_newline.txt");
        assert_eq!(result, Wc { rows: 1, words: 2, bytes: 12});

        let result = wc("files/spaces.txt");
        assert_eq!(result, Wc { rows: 3, words: 7, bytes: 47});
